...
def siparis_olustur(
    ...
    return services.siparis_service.create_order_from_cart(db=db, kullanici=current_user, siparis_data=siparis_data)
...
def kullanici_siparislerini_listele(
    ...
    return services.siparis_service.get_kullanici_siparisleri(db=db, kullanici_id=current_user.id)

@router.get("/{siparis_id}/fatura", response_class=StreamingResponse)
def faturayi_indir(
    siparis_id: int,
    db: Session = Depends(dependencies.get_db),
    current_user: models.User = Depends(dependencies.get_current_user),
