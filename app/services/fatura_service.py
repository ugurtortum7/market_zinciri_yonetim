...
def generate_invoice_pdf_in_memory(siparis: models.Siparis) -> BytesIO:
    """
    Verilen sipariş için bir PDF faturasını hafızada (in-memory) oluşturur.
    """
    try:
        buffer = BytesIO()
        ...
        # Standart fontlar kullanarak Türkçe karakter sorununu en aza indiriyoruz
        ...
        for detay in siparis.detaylar:
            urun_adi = detay.urun.urun_adi if detay.urun else "Bilinmeyen Urun"
        ...
        buffer.seek(0)
        log.info(f"Siparis {siparis.id} icin fatura hafizada basariyla olusturuldu.")
        return buffer
